...
class Server:
    ...
    def setup_routes(self):
        ...
        self.app.add_url_rule("/", "index_page", self.index_page)
        self.app.add_url_rule("/<path:alias>", "getpage", self.getpage)
    ...
    def getpage(self, alias:str):
        ...
        if is_debugging():
            ...
            if realip := request.headers.get('X-Real-IP'):
                logger.debug("x_real_ip: %s", realip)
        if alias.endswith('/version') or alias == 'version':
            ...
            return self.projapi.get_version(alias,request) # 获取版本号
        ...
        try:
            if mode == 'json':
                response_dict = self.projapi.get_page_json(alias)
                logger.debug(t("server.request.response.json",page=alias,args=args))
            else:
                client = PCLClient.from_request(web_request=request)
                logger.debug(t("server.request.version", version=str(client.version)))
                response_dict = self.projapi.get_page_response(alias, client, args=args)
            ...
            return self.process_not_found(alias,mode)
        except Exception as e:
            logger.error(t("server.request.response.error",page=alias,args=args))
            logger.exception(e)
            if mode == 'json':
                return self.process_err_page_json(500)
            else:
                return 'Inner Error Occured',500

    def process_not_found(self, alias,mode):
        '''处理页面未找到的请求'''
        if mode == 'json':
            return self.process_err_page_json(404)
        if alias == 'could_you_buy_me_a_coffee':
            return 'Sure You Need A Coffee And I AM A TEAPOT',418
        return 'No Page Found',404

    def process_err_page_json(self, err_code):
        '''处理发生错误的 JSON 请求'''
