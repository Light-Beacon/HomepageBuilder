...
class TemplateManager():
    ...
    def expend_card_placeholders(self,card:dict,children_code):
        ...
        while not q.empty():
            ...
            key = q.get()
            try:
                card[key] = format_code(card[key], card, children_code=children_code)
                tries = 0
    ...
    def build_with_template(self,card,template_name,children_code) -> str:
        ...
        for cpn in target_template['components']:
            ...
            if cpnobj := context.components.get(cpn):
                code += cpnobj.toxaml(card,children_code)
            elif cpn.startswith('$') or cpn.startswith('@'):
                args = cpn[1:].split('|')
                code += invoke_script(args[0],card=card,args=args[1:],children_code=children_code)
            elif not cpn == '':
            ...
            match container_name:
                ...
                case _:
                    if container_object := context.components.get(container_name):
                        current_code = container_object.toxaml(card, current_code)
                    else:
